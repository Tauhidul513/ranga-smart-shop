...
const showProducts = (products) => {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <p>${averageRate(product.rating.rate)}</p>
      ...
      <button data-target="#myModal" data-toggle="modal" class="details-button btn btn-success">Details</button></div>
      `;
        document.getElementById("all-products").appendChild(div);
    }
};
let count = 0;
//add to cart function and related calculation
const addToCart = (id, price) => {
    ...
    // Updating total price function call
    updateTotal();
};
// common getinputvalue function
...
const averageRate = (rate) => {
    ...
    const remainder = (rate % 1).toFixed(2);
    if (remainder >= 0.3 && 0.7 > remainder) {
